...
/**
 * Returns the flex basis that each participant should use
 * to achieve the desired layout.
 *
 * Since the flex basis only applies across the flex-direction
 * it needs to be decided carefully so as to also achieve the
 * desired number of rows and pushes the right number onto
 * the bottom row
 */
function flexContainerWidth(users: number, height: number, width: number) {
	const targetAspectRatio = 4 / 3

	let numRows = 1
	let numCols = 1

	while (numRows * numCols < users) {
		// find which propsal leads to the smallest size change for items
		const cellAspectRatioWithAddedCol =
			width / (numCols + 1) / (height / numRows)
		const cellAspectRatioWithAddedRow =
			width / numCols / (height / (numRows + 1))

		const colDiff = Math.abs(targetAspectRatio - cellAspectRatioWithAddedCol)
		const rowDiff = Math.abs(targetAspectRatio - cellAspectRatioWithAddedRow)
	...
	// bring numCols back down if needed
	if (numCols > numRows) {
		numCols = Math.ceil(users / numRows)
	}
