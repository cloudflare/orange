...
var A = function (A) {
	...
	var f,
		P,
		i = Q.printErr || console.warn.bind(console)
	Object.assign(Q, B),
		(B = null),
	...
	function n(A, g) {
		...
		try {
			;(B = (function (A) {
				try {
					if (A == G && f) return new Uint8Array(f)
					var g = u(A)
					if (g) return g
					if (D) return D(A)
				...
				(Q = new WebAssembly.Module(B)),
				(I = new WebAssembly.Instance(Q, g))
		} catch (A) {
			var C = A.toString()
			throw (
				(i('failed to compile wasm module: ' + C),
				(C.includes('imported Memory') || C.includes('memory import')) &&
					i(
	...
	function t(A) {
		try {
			return P.grow((A - H.byteLength + 65535) >>> 16), s(P.buffer), 1
		} catch (A) {}
	}
	S(
		(G =
	...
	) || ((r = G), (G = Q.locateFile ? Q.locateFile(r, w) : w + r))
	var c =
		'function' == typeof atob
			...
			: function (A) {
					...
					do {
						;(g =
							(D.indexOf(A.charAt(f++)) << 2) |
							((B = D.indexOf(A.charAt(f++))) >> 4)),
							(I = ((15 & B) << 4) | ((C = D.indexOf(A.charAt(f++))) >> 2)),
							(Q = ((3 & C) << 6) | (E = D.indexOf(A.charAt(f++)))),
							(w += String.fromCharCode(g)),
							64 !== C && (w += String.fromCharCode(I)),
							64 !== E && (w += String.fromCharCode(Q))
					} while (f < A.length)
					return w
				}
	function u(A) {
		if (S(A))
			return (function (A) {
				try {
					for (
						var g = c(A), I = new Uint8Array(g.length), Q = 0;
						Q < g.length;
						++Q
					)
						I[Q] = g.charCodeAt(Q)
			...
			a: function (A) {
				...
				for (var C = 1; C <= 4; C *= 2) {
					...
					if (
						...
						t(
							Math.min(B, (g = Math.max(A, E)) + (((I = 65536) - (g % I)) % I))
						))
					)
						return !0
				}
		...
		k = (function () {
			...
			function g(A, g) {
				...
				;(Q.asm = B),
					...
					(function (A) {
						if (
							(e--,
							Q.monitorRunDependencies && Q.monitorRunDependencies(e),
							0 == e && R)
						) {
							var g = R
							;(R = null), g()
			...
			if (
				...
				try {
					...
					return (
						i('Module.instantiateWasm callback failed with error: ' + A), !1
					)
				}
			return g(n(G, A)[0]), Q.asm
		})()
	function T(A) {
		function I() {
			O ||
	...
	if (
		((Q.___wasm_call_ctors = k.d),
		(Q._rnnoise_init = k.e),
		(Q._rnnoise_create = k.f),
		(Q._malloc = k.g),
		(Q._rnnoise_destroy = k.h),
		(Q._free = k.i),
		(Q._rnnoise_process_frame = k.j),
		(R = function A() {
			O || T(), O || (R = A)
		}),
		(Q.run = T),
...
class I {
	...
	processAudioFrame(A, I = !1) {
		for (let I = 0; I < g; I++)
			this._wasmInterface.HEAPF32[this._wasmPcmInputF32Index + I] = 32768 * A[I]
		const Q = this._wasmInterface._rnnoise_process_frame(
			this._context,
			this._wasmPcmInput,
			this._wasmPcmInput
		)
		if (I)
			for (let I = 0; I < g; I++)
				A[I] =
					this._wasmInterface.HEAPF32[this._wasmPcmInputF32Index + I] / 32768
		return Q
	}
}
class Q {
	constructor(A, g) {
		;(this._procNodeSampleRate = 128),
			...
			(this._denoiseProcessor = A),
			(this._denoiseSampleSize = g),
	...
	process(A, g) {
		if (!A) return
		for (
			this._circularBuffer.set(A, this._inputBufferLength),
				this._inputBufferLength += A.length;
		...
		if (
			...
			const A = this._circularBuffer.subarray(
				...
				this._denoisedBufferIndx + g.length
			)
			g.set(A, 0), (this._denoisedBufferIndx += g.length)
		}
...
class B extends AudioWorkletProcessor {
	constructor() {
		...
		const g = new I(A()),
			...
			C = g.getSampleLength(),
			E = B.getSampleLength()
		;(this.leftChannel = new Q(g, C)), (this.rightChannel = new Q(B, E))
	}
	process(A, g) {
		const I = A[0][0],
			Q = A[0][1],
		...
		return (
			!I ||
			!Q ||
			(this.leftChannel.process(I, B), this.rightChannel.process(Q, C), !0)
		)
	}
}
registerProcessor('NoiseSuppressorWorklet', B)
